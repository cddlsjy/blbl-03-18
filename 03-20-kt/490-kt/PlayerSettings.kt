...
internal object PlayerSettingKeys {
    ...
    const val GESTURE_DIRECTION_REVERSED = "gesture_direction_reversed"
}
...
internal fun PlayerActivity.handleSettingsItemClick(item: PlayerSettingsAdapter.SettingItem) {
    when (item.key) {
        ...
        PlayerSettingKeys.GESTURE_DIRECTION_REVERSED -> {
            val appPrefs = BiliClient.prefs
            appPrefs.playerGestureDirectionReversed = !appPrefs.playerGestureDirectionReversed
            (binding.recyclerSettings.adapter as? PlayerSettingsAdapter)?.let { refreshSettings(it) }
        }
...
internal fun PlayerActivity.refreshSettings(adapter: PlayerSettingsAdapter) {
    ...
    val items =
        buildList {
            ...
            add(
                PlayerSettingsAdapter.SettingItem(
                    key = PlayerSettingKeys.GESTURE_DIRECTION_REVERSED,
                    title = "手势滑动方向反向",
                    subtitle = if (prefs.playerGestureDirectionReversed) "开" else "关",
                ),
            )
            add(
                PlayerSettingsAdapter.SettingItem(
