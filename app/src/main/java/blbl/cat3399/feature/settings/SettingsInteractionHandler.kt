...
class SettingsInteractionHandler(
    ...
    fun onEntryClicked(entry: SettingEntry) {
        ...
        when (entry.id) {
            ...
            SettingId.GestureDirectionReversed -> {
                prefs.playerGestureDirectionReversed = !prefs.playerGestureDirectionReversed
                AppToast.show(activity, "手势滑动方向反向：${if (prefs.playerGestureDirectionReversed) "开" else "关"}")
                renderer.refreshSection(entry.id)
            }
